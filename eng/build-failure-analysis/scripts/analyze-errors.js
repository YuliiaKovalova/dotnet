...
// Adapted from microsoft/testfx PR #8326 for dotnet/dotnet (VMR).
// Reads /tmp/binlog-analysis.json, calls LLM, writes /tmp/analysis-result.md.
//
// Key differences from testfx version:
//  - Handles aggregated errors from multiple binlogs / build legs
//  - Uses Azure DevOps build URL instead of GitHub Actions run URL
//  - Tailored prompt for VMR / insertion failure patterns
...
// --- Build AI prompt (tailored for VMR insertions) ---

const prompt = [
  'You are an MSBuild build failure analyst for the dotnet/dotnet VMR (Virtual Monolithic Repository).',
  'This build failure was triggered by a Maestro/Darc component insertion PR.',
  'Common failure patterns include:',
  '- Version conflicts between updated and existing packages',
  '- API breaking changes in an upstream component',
  '- Missing or mismatched TargetFramework references',
  '- Source-build compatibility issues',
  '- Transitive dependency downgrades (NU1605)',
  '',
  'Analyze the following build failure data and provide:',
  '1. A concise summary of what failed and which component insertion likely caused it',
  ...
  '4. If the fix requires changes in the upstream component repo (not dotnet/dotnet), note that clearly',
  '',
...
if (data.warnings && data.warnings !== 'No data' && data.warnings !== 'N/A') {
